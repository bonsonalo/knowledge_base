...
@router.get("/self_articles", response_model=list[ArticleResponse])
...
@router.get("/single_article_editor/{article_id}", response_model=ArticleResponse)
async def get_article_editor(article_id: UUID, current_user: editor_dependency, db: db_dependency):
    try:
        return await get_article_editor_service(article_id, current_user, db)
    except ValueError as e:
        logger.error(str(e))
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail= str(e))
...
@router.patch("/change_status/{article_id}")
async def change_article_status(article_id: UUID, new_status: str, current_user: editor_dependency, db: db_dependency):
    try:
        return await change_article_status_service(article_id, new_status, current_user, db)
    except ValueError as e:
        logger.error(str(e))
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail= str(e))
